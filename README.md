...
- Python 3.10 또는 3.11
- 공식 평가 서버: Python 3.11, Linux, CPU 환경
...
## 2. 공식 평가 환경
공식 평가는 Python 3.11 기반 Linux Docker CPU 환경에서 실행됩니다.

Windows나 macOS에서 로컬 테스트를 통과하더라도 운영체제 차이로 공식 평가가 실패할 수 있습니다.
제출 전 다음 사항을 반드시 확인하십시오.

- 파일명과 코드에서 사용하는 파일명의 대소문자가 정확히 일치해야 합니다.
- Windows 전용 경로나 절대 경로를 사용하지 마십시오.
- 모델 파일은 `torch.load("model.pth", map_location="cpu")`와 같이 상대 경로로 불러오십시오.
- 추론에 필요한 추가 패키지는 `requirements.txt`에 정확한 버전으로 작성해야 합니다.
- 추가 패키지는 Python 3.11 및 Linux 환경에서 별도 컴파일 없이 설치할 수 있어야 합니다.
- GPU 없이도 모델을 불러오고 추론할 수 있어야 합니다.

로컬 실행 성공은 공식 평가 성공을 보장하지 않습니다. 최종 제출물은 반드시 공식
Linux Docker 환경과 호환되어야 합니다.
...
- 파일명과 코드에서 사용하는 파일명의 대소문자가 일치하는가?
- Windows 전용 경로나 절대 경로를 사용하지 않았는가?
- CPU 환경에서 모델을 로드하고 추론할 수 있는가?
- 추가 패키지를 `requirements.txt`에 정확한 버전으로 명시했는가?
- 추가 패키지가 고정 라이브러리 버전과 호환되는가?
- 추가 패키지가 Python 3.11 및 Linux 환경에서 별도 컴파일 없이 설치 가능한가?
