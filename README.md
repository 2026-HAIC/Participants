...
트랙을 100스텝(행동 기준) 연속으로 벗어나 있으면(정지·역주행 포함) 오프트랙으로
리타이어됩니다. 트랙에 복귀해 새 타일을 밟으면 카운트는 즉시 초기화됩니다.

## 점수 계산

`local_runner.py`가 출력하는 최종 점수는 공식 채점과 동일한 공식을 씁니다.

- 완주(트랙 타일의 95% 이상 방문): 점수 = 걸린 스텝 수
- 미완주(리타이어 또는 `max-steps` 소진): 점수 = -진행률 (0.0 ~ -1.0)

`local_runner.py`가 매 스텝 출력하는 누적 보상은 위 점수와 다른 값입니다 -- 원본
CarRacing 환경의 보상(매 프레임 -0.1, 타일 방문 시 +1000/N)이며, 로컬 학습 시
신호로 참고할 수는 있지만 실제 채점 기준은 아닙니다.
