...
from core.track_variables import (
    TrackVariables,
    build_track_variables,
    validate_track_id,
    with_obstacles,
)
...
class VariablesContactDetector(FrictionDetector):
    ...
    def _match_obstacle_contact(self, contact):
        if self.env.car is None:
            return None
        car_bodies = (self.env.car.hull, *self.env.car.wheels)
        return match_obstacle_body(
...
class CarRacing(gym.Env, EzPickle):
    ...
    def __init__(
        ...
        grass_friction_multiplier: float = 0.6,
    ):
        EzPickle.__init__(
            self,
            render_mode,
        ...
        self.track_seed: Optional[int] = None
        self.track_variables: Optional[TrackVariables] = None
        self._init_colors()

        self.contactListener_keepref = VariablesContactDetector(self, self.lap_complete_percent)
    ...
    def _detach_contact_listener(self):
        """Detach native callbacks before destroying any contacted bodies."""
        self.world.contactListener = None
        self.world.contactListener_bug_workaround = None
        self.contactListener_keepref = None

    def _attach_contact_listener(self):
        """Install and retain the listener used by the next physics step."""
        listener = VariablesContactDetector(self, self.lap_complete_percent)
        self.world.contactListener_bug_workaround = listener
        self.world.contactListener = listener

    def _destroy(self):
        self._detach_contact_listener()
    ...
    def _destroy_obstacles(self):
        for index in range(len(self.obstacles) - 1, -1, -1):
            self.world.DestroyBody(self.obstacles[index])
            del self.obstacles[index]

    def _create_obstacles(self, specs):
        for spec in specs:
            body = self.world.CreateStaticBody(position=spec.position)
            body.CreateCircleFixture(radius=spec.radius, isSensor=False)
            body.userData = ObstacleUserData()
    ...
    def reset(
        ...
        *,
        ...
        track_id = reset_options.get("track_id")

        # Official scoring/replay always supplies track_id. Keeping it optional
        # preserves the normal Gymnasium reset contract for local callers, but
        # missing context must clear (never reuse) the previous official track.
        self.track_id = None
        self.track_seed = None
        self.track_variables = None
        if track_id is not None:
            track_id = validate_track_id(track_id)
            if isinstance(seed, bool) or not isinstance(seed, (int, np.integer)):
                raise TypeError("seed must be an integer when options['track_id'] is provided")
            seed = int(seed)
            ...
            self.track_variables = build_track_variables(track_id=track_id, seed=seed)
            self.grass_friction_multiplier = self.track_variables.grass_friction_multiplier
        self._destroy()
        self._attach_contact_listener()
        self.reward = 0.0
        self.prev_reward = 0.0
        ...
        if self.track_variables is not None:
            assert self.track_id is not None and self.track_seed is not None
            self.track_variables = with_obstacles(
            ...
            self._create_obstacles(self.track_variables.obstacles)
        self.car = Car(
    ...
    def _render_obstacles(self, zoom, translation, angle):
        for body in self.obstacles:
            ...
            center_x, center_y = body.position
            polygon = [
                (
                    center_x + radius * math.cos(2 * math.pi * index / OBSTACLE_RENDER_VERTICES),
                    center_y + radius * math.sin(2 * math.pi * index / OBSTACLE_RENDER_VERTICES),
                )
                for index in range(OBSTACLE_RENDER_VERTICES)
            ]
            self._draw_colored_polygon(
                self.surf, polygon, OBSTACLE_COLOR, zoom, translation, angle
            )
